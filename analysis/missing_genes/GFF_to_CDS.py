...
This file reads through a gff and creates a new gff with only the lines that pertain to the CDS of protein coding genes.
@author: phil-grayson
"""
...
with open('/Users/Phil/Desktop/PseudoSearch/GCF_000002315.3_Gallus_gallus-4.0_genomic.gff', 'rU') as handle: #opens gff in universal mode
    ...
    for lll in reader:
        if len(lll)==9: #only the lines in proper gff format (length of 9) should be examined
            ...
            if bool(re.search(r'GeneID\:(\d*)\;.*gene_biotype=protein_coding', lll[8])): #if protein coding biotype in line
