...
#alignment file
my $aln= 'introns_mafft_concat.phylip';

#partition file
my $partitions= 'introns_mafft_concat_partitions';
...
my @glob3= <introns_mafft_concat.phylip.BS*>;
for my $path3 (@glob3){
	unlink($path3);
}
...
for (my $i= 0; $i<= 199; $i++){
	...
	my $parseexaml= "parse-examl -s introns_mafft_concat.phylip.BS$i -q $partitions -m DNA -n run1_BS$i\_aln";
	system($parseexaml);
}
