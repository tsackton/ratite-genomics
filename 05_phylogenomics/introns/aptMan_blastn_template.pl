...
#parse the batch from the end of the input fasta name
#NB- full set of queries split into 'batches' of ~1000 seqs.
my $batch;
if ($fastafile=~ /(blastn_queries_)(\d+)(\.fasta)$/){
	$batch= $2;
}
else{
	print "Error parsing 'batch' from: $fastafile\n";
	die;
}

#for stops/restarts on serial requeue partition
my $logfile= $refspp . '-' . $queryspp . '_' . $batch . '_logfile';
...
my $file= "$refspp-$queryspp\_blastn_results_$batch";
if (-e $file){
...
while (my $seqobj= $seqio_in-> next_seq()){
	...
	if ($qid=~ /(batch)(\d+?)(_)(.+)$/){
		...
		print "Error parsing locus from: $qid\n";
		die;
	}
	$query_count++;
