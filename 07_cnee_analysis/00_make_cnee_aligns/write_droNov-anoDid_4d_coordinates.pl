...
#store the start & end coordinates for each droNov reference seq., and the strand
print "Parsing coordinate info. for droNov...\n";

my $bedfile= shift(@ARGV);
#parse the list# (b/c full droNov .bed was split into multiple input files to run in parallel)
my $listnum;
if ($bedfile=~ /(list)(\d+)$/){
	$listnum= $2;
}
else{
	print "Error parsing list # from: $bedfile\n";
	die;
}
my %ref;
my %starts;
...
print "Stored: $stored seqs. to map to moa coords (skipped $skipped missing/multiple in droNov)\n\n";
...
my $outfile= 'moa-droNov_coordinates_to_retrieve_' . $listnum;
