...
gwide <- orig_gl %>% filter(rar_qval < 0.01, window_size == "1000kb_100kb_slide") %>% summarize(maxpval=max(rar_pval)) %>% pull(maxpval)
ss <- orig_gl %>% filter(rar_qval < 0.05, window_size == "1000kb_100kb_slide") %>% summarize(maxpval = max(rar_pval)) %>% pull(maxpval)

#convert to format for qqman
se_manh <- orig_gl %>% filter(window_size == "1000kb_100kb_slide") %>% select(chr, start, end, rar_pval, window) %>%
...
orig_gm %>% filter(window_size == "1000kb_100kb_slide") %>% mutate(RAR = rar_pval < ss, CRAR = crar_pval < ss, logp = -log10(rar_pval)) %>% filter(RAR | CRAR) %>% select(chr, start, end, RAR, CRAR, logp) %>% View()
