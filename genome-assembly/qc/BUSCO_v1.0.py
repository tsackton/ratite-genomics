...
try:
  if args['clade']!=None:
    ...
    if clade.startswith(('m','M')):
      clade='metazoa';Z=91897;maxflank=20000
    elif clade.startswith(('a','A')):
      clade='arthropoda';Z=102785;maxflank=20000
    elif clade.startswith(('v','V')):
      clade='vertebrata';Z=143785;maxflank=30000
    elif clade.startswith(('f','F')):
      clade='fungi';Z=174195;maxflank=20000
    elif clade.startswith(('eu','Eu','E')):
      clade='eukaryota';Z=41317;maxflank=20000
    elif clade.startswith(('example','ex')):
      clade='example';Z=102785;maxflank=20000
      ...
      try:
        ...
        maxflank=20000
        print('Using default maximum flank size (20000 bp)')
except:
  print('Please indicate a BUSCO clade: (E)ukaryota, (M)etazoa, (A)rthropoda, (V)ertebrata or (F)ungi')
  raise SystemExit
  ...
  for i in dic.keys():
    if len(dic[i])>1:
       for z in range(0,len(dic[i])):
         os.system('augustus --proteinprofile=./%(clade)s/%(prot_profile)s --predictionStart=%(start_coord)s --predictionEnd=%(end_coord)s --species=%(species)s \"%(scaffold)s\" > %(output)s' % 
	        {'prot_profile':'prfl/'+i+'.prfl','start_coord':dic[i][z][1],'end_coord':dic[i][z][2],
	  'clade':clade,'species':target_species,'scaffold':dic[i][z][0]+args['abrev']+'_.temp','output':mainout+'augustus/'+i+'.out.'+str(z+1)})
    else:
      os.system('augustus --proteinprofile=./%(clade)s/%(prot_profile)s --predictionStart=%(start_coord)s --predictionEnd=%(end_coord)s --species=%(species)s \"%(scaffold)s\" > %(output)s' % 
	      {'prot_profile':'prfl/'+i+'.prfl','start_coord':dic[i][0][1],'end_coord':dic[i][0][2],
	'clade':clade,'species':target_species,'scaffold':dic[i][0][0]+args['abrev']+'_.temp','output':mainout+'augustus/'+i+'.out'})
...
if mode=='genome' or mode=='hmmer':  #should be augustus
  ...
  for i in files:
    if args['species']!='brugia':
      os.system('sed -i \'1,18d\' %saugustus/%s' % (mainout,i)) 
    else:
      os.system('sed -i \'1,15d\' %saugustus/%s' % (mainout,i)) 
  ...
  for item in re_run:
    if item not in dic: #no coordinates found
      ...
      for entry in dic[item]:
        count+=1;
        os.system('augustus --proteinprofile=./%(clade)s/%(prot_profile)s.prfl --predictionStart=%(start_coord)s --predictionEnd=%(end_coord)s --species=%(species)s \"%(scaffold)s\" > %(output)s' % 
	    {'prot_profile':'prfl/'+item,'start_coord':entry[1],'end_coord':entry[2],'clade':clade,'species':target_species,'scaffold':entry[0]+args['abrev']+'_.temp','output':mainout+'augustus/'+item+'.out.'+str(count)})
        os.system('sed -i \'1,18d\' %(group_name)s' % {'group_name':mainout+'augustus/'+item+'.out.'+str(count)});
        addendum=item+'.out.'+str(count);
        extract(mainout,addendum)
        os.system('hmmsearch --domtblout %(output_file)s -Z %(db_size)s -o temp --cpu %(cpu)s %(group_file)s.hmm %(input_file)s ' % 
	    {'input_file':mainout+'/augustus_proteins/'+item+'.fas.'+str(count),'db_size':Z,'cpu':cpus,'group_file':clade+'/hmms/'+item,'output_file':mainout+'hmmer_output/'+item+'.out.'+str(count)})
    elif len(dic[item])==1:
      entry=dic[item][0]
      try:
        os.system('augustus --proteinprofile=./%(clade)s/%(prot_profile)s.prfl --predictionStart=%(start_coord)s --predictionEnd=%(end_coord)s --species=%(species)s \"%(scaffold)s\" > %(output)s' % 
	    {'prot_profile':'prfl/'+item,'start_coord':entry[1],'end_coord':entry[2],'clade':clade,'species':target_species,'scaffold':entry[0]+args['abrev']+'_.temp','output':mainout+'augustus/'+item+'.out'})
        os.system('sed -i \'1,18d\' %(group_name)s' % {'group_name':mainout+'augustus/'+item+'.out'})
        extract(mainout,item);name=item
        os.system('hmmsearch --domtblout %(output_file)s.out -Z %(db_size)s -o temp --cpu %(cpu)s %(group_file)s.hmm %(input_file)s.fas' % 
	    {'input_file':mainout+'/augustus_proteins/'+name,'db_size':Z,'cpu':cpus,'group_file':clade+'/hmms/'+name,'output_file':mainout+'hmmer_output/'+name})
      except:
        pass   
...
#summarize results, print and write to output files
  ...
  print('Total complete BUSCOs found in assembly (<2 sigma) :  %s\t(%s duplicated).' % (len(set(unique)),len(mcc)))
  print('Total BUSCOs partially recovered (>2 sigma) :  %s' % (fcc))
  print('Total groups searched: {}'.format(totalbuscos))
  try:
    print('Total BUSCOs not found:  %s' % (totalbuscos-(len(set(cc))+fcc)))
  except:
    print('Total BUSCOs not found:  %s' % (totalbuscos-(len(set(cc))+len(fcc))))
  ...
  summary.write('#Summarized BUSCO benchmarking for file: %s\n#BUSCO was run in mode: %s\n\n' % (args['genome'],mode))
  summary.write('Summarized benchmarks in standard BUSCO notation:\n\tC:{:.2%}[D:{:.2%}],F:{:.2%},M:{:.2%},n:{:d}\n\n'.format(len(set(cc))/float(totalbuscos),len(set(mcc))/float(totalbuscos),fcc/float(totalbuscos),(float(totalbuscos)-(len(set(cc))+fcc))/float(totalbuscos),int(totalbuscos)))

  summary.write('Representing:\n')
  summary.write('\t%s\tComplete Single-Copy BUSCOs\n' % (len(set(cc))))
  summary.write('\t%s\tComplete Duplicated BUSCOs\n' % (len(set(mcc))))
  summary.write('\t%s\tFragmented BUSCOs\n' % (fcc))
  summary.write('\t{:d}\tMissing BUSCOs\n'.format(int(totalbuscos)-(len(set(cc))+fcc)))
