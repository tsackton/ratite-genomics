...
my $min_seq_size; #minimum size of sequence to count in stats
...
GetOptions ("limit=i"       => \$limit,
			...
			"min_seq=i" => \$min_seq_size);

# set defaults
$limit = 1000000000 if (!$limit);
$n_limit = 25       if (!$n_limit);
$min_seq_size = 1000 if (!$min_seq_size);
...
sub process_FASTA{
	...
	while(my $entry = $fasta->nextEntry){
		...
		next unless $length >= $min_seq_size;
		$seq_count++;
...
sub sequence_statistics{
	...
	my $n50_size = sum(grep {$_ >= 1000} @{$data{$type}{lengths}});
	...
	if ($type eq 'scaffold'){		
		...
		$desc = "Useful amount of $type sequences (>= 100K nt)";
		printf "%${w}s %10d\n", $desc, $sum_useful;
	...
	foreach my $length (reverse sort{$a <=> $b} @{$data{$type}{lengths}}){
		...
		while($running_total > int (($n_index / 100) * $n50_size)){	
			if ($n_index == 50){
				$n50_length = $length;
